...
- (id)initWithPoint:(CGPoint)point radius:(int)radiusValue inView:(UIView *)inView
{
    ...
    self.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.005];
    return self;
}
...
-(void)show
{
    if(!self.isAnimating)
    {
        ...
        if(self.showTwitterBubble) {
            UIButton *twitterBubble = [self shareButtonWithIcon:@"icon-share.png" andBackgroundColorRGB:self.twitterBackgroundColorRGB];
        ...
        if(self.showGooglePlusBubble) {
            UIButton *googlePlusBubble = [self shareButtonWithIcon:@"icon-share.png" andBackgroundColorRGB:self.googlePlusBackgroundColorRGB];
